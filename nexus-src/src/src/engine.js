...
function computeTableForTrack(t, droughts) {
  if(t.type==="fibonacci") {
    const cfg = t.config;
    // If Follow the Leader, resolve dynamic targets for table shape
    const resolved = droughts ? resolveDynamicTargets(cfg, droughts) : {
      dozenTargets: cfg.dozenTargets||[], colTargets: cfg.colTargets||[], evenTargets: cfg.evenTargets||[]
    };
    const dts=resolved.dozenTargets, cts=resolved.colTargets, evts=resolved.evenTargets;
    const betMode=cfg.betMode||"progression";
    const effectiveBetMode = betMode==="flat" ? "flat" : (
      // Even money without FTL: martingale. With FTL: still martingale (1 bet at a time)
      (evts.length>0||(cfg.evenTargets||[]).length>0) ? "martingale" : "progression"
    );
    const maxChips=cfg.stopLoss/(cfg.unit||1);
    if(effectiveBetMode==="flat") {
      const payMult=(evts.length>0||(cfg.evenTargets||[]).length>0)?2:3;
      const subs=(evts.length||(dts.length+cts.length))||1;
      return [{level:1,c:1,totalBet:subs,totalInvest:subs,ret:payMult,profit:payMult-subs,roi:(payMult-subs)/subs*100,flat:true}];
    }
    if(evts.length>0||(cfg.evenTargets||[]).length>0) {
      if(effectiveBetMode==="martingale") return computeMartingaleTable(maxChips);
      return computeTable(evts.length||1,2,cfg.roi,maxChips);
    }
    const subs=dts.length+cts.length, hasMix=dts.length>0&&cts.length>0;
    return computeTable(subs||1,hasMix?6:3,cfg.roi,maxChips);
  }
